...
def update_result2(p1, p2, p3, p4, p5):
    global conn
    global cursor
    if conn is None:
        conn = psycopg2.connect(
    ...
    if cursor is None:
        cursor = conn.cursor()
        print("have got cursor again")

    try:
        print("P1 >>> ", p1)
        if p1 == 2:
            cursor.execute("""
                UPDATE retrieve_nums4
                SET search_word1 = %s, 
                    con_and = %s,
                    update_datetime = now()
                WHERE name_ko = %s
            ...
            print("update established (", p2, "=", p3, ") at ", p4, "&&", p5)
        elif p1 == 3:
            cursor.execute("""
                UPDATE retrieve_nums4
                SET search_word2 = %s, 
